...
function isNode(u: unknown): u is Node {
  return typeof u === "object" && u != null && "ELEMENT_NODE" in u
}

function isHTMLOrSVGElement(u: unknown): u is HTMLOrSVGElement {
  return typeof u === "object" && u != null && "dataset" in u
}
...
function synchronize(
  ...
  never,
  Hole.NoNextSiblingException | Wire.NoFirstChildException | Wire.NoLastChildException,
  Array<Node | Wire>
> {
  return whileDiscard({
    bLength: b.length,
        ...
        s.bEnd > s.bLength ?
          (s.bStart ?
            get(Maybe.fromNullable(b[s.bStart - 1]), -0).flatMap(({ nextSibling }) =>
              Effect.fromMaybe(Maybe.fromNullable(nextSibling)).mapError(() => new Hole.NoNextSiblingException())
            ) :
...
function diff(comment: Node, oldNodes: Array<Node | Wire>, newNodes: Array<Node | Wire>): Effect<
  never,
  Hole.NoNextSiblingException | Wire.NoFirstChildException | Wire.NoLastChildException | Hole.NoParentNodeException,
  Array<Node | Wire>
> {
  return Effect.fromMaybe(Maybe.fromNullable(comment.parentNode)).mapError(() => new Hole.NoParentNodeException())
    .flatMap((parentNode) =>
      synchronize(
        parentNode,
...
function anyContent(
  ...
  never,
  | Hole.NoNextSiblingException
  | Wire.NoFirstChildException
  | Wire.NoLastChildException
  | Hole.NoParentNodeException
  | Hole.NoTextNodeException,
  void
> {
      ...
      switch (typeof newValue) {
        ...
        case "boolean":
          if (oldValue != Maybe.some(newValue)) {
            ...
            ).flatMap((_) => Effect.fromMaybe(_).mapError(() => new Hole.NoTextNodeException())).flatMap((text) =>
...
function data(node: Element) {
  ...
  return Effect.fail(new Hole.InvalidElementException())
}
...
function handlers(fragment: DocumentFragment) {
  return ({ name, path, type }: Template.Node) => {
    const node = path.reduceRight(
      Maybe.some<Node>(fragment),
      (i, childNode) => childNode.flatMap(({ childNodes }) => Maybe.fromNullable(childNodes[i]))
    )

    if (node.isNone()) {
      throw new Error(`Unable to acquire node, probably because of an outside change`)
    }

    return type === "node" ?
      handleAnything(node.value) :
      (type === "attr" ?
        handleAttribute(node.value, name) :
        text(node.value))
  }
}
...
export function toEntry(
  ...
  TemplateCache.getOrElse(self.template, self.toTemplate).map((template) => {
    // clone deeply the fragment
    // and relate an update handler per each node that needs one
    const updates = nodes.map(handlers(template.toDocumentFragment))
    // return the fragment and all updates to use within its nodes
    return { content: fragment, updates }
  })

  return Entry(self.type, self.template)
}
